import { allowMethods, cleanText, db, handleError, json, rateLimit, requireUser, safeEmail, safeUrl, trustedMediaUrl } from './_lib.js';
...
export default async function handler(req,res){
 ...
 try{
  const user=await requireUser(req);await rateLimit(req,{key:'manage-app',limit:30,windowSeconds:600,userId:user.id});const id=String(req.query?.id||'');
  ...
  if(b.contact_email!=null)patch.contact_email=safeEmail(b.contact_email);
  ...
  if(b.icon_url!=null)patch.icon_url=trustedMediaUrl(b.icon_url,user.id);
  if(Array.isArray(b.screenshot_urls)){if(b.screenshot_urls.length<1||b.screenshot_urls.length>3)return json(res,400,{error:'Upload between 1 and 3 screenshots'});patch.screenshot_urls=b.screenshot_urls.map(v=>trustedMediaUrl(v,user.id))}
