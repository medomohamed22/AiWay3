...
function requestOrigin(req){const proto=String(req.headers['x-forwarded-proto']||'https').split(',')[0].trim();const host=String(req.headers['x-forwarded-host']||req.headers.host||'').split(',')[0].trim();return host?`${proto}://${host}`:''}
function applyCors(req,res){const origin=req.headers.origin||'',own=requestOrigin(req);if(origin&&origin===own)res.setHeader("Access-Control-Allow-Origin",origin);res.setHeader("Vary","Origin");res.setHeader("Access-Control-Allow-Headers","Content-Type, Authorization");res.setHeader("Access-Control-Allow-Methods","POST, OPTIONS")}
function isOriginAllowed(req){const origin=req.headers.origin||'';return !origin||origin===requestOrigin(req)}
...
export default async function handler(req,res){
  ...
  try{
    ...
    const upstream=await fetch(OPENROUTER_CHAT_URL,{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json","HTTP-Referer":requestOrigin(req)||"https://aiway.app","X-OpenRouter-Title":process.env.APP_NAME||"AiWay"},body:JSON.stringify({model,stream:true,messages:[{role:"system",content:systemText},...messages],temperature:.7,max_tokens:8192,plugins:[{id:"file-parser"}]}),signal:AbortSignal.timeout(55000)});
  ...
  }catch(error){if(!res.headersSent){const status=error.status||(/غير صالح|غير مسموح|عدد الرسائل/.test(error.message)?400:500);return res.status(status).json({error:status===401?'يجب تسجيل الدخول بحساب Pi أولًا.':status===400?error.message:"حدث خطأ أثناء الاتصال بالنموذج."})}res.end()}
}
